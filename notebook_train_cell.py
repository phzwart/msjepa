...
#
# For DDP (4 GPUs): set use_ddp=True, num_gpus=4 and either:
#   - train_root=..., val_root=... (folder-based), or
#   - data_pt_path=... (single .pt file; optional val_pt_path=... or val_fraction=0.1 to split)
# Workers live in msjepa.notebook_ddp so spawn can pickle them when running from a notebook.
...
from msjepa.notebook_ddp import run_ddp_worker_pt, run_ddp_worker_folders
...
# --- Setup: DDP vs single-device ---
# For DDP: use_ddp=True, num_gpus=4 and either train_root+val_root or data_pt_path (optional val_pt_path or val_fraction).
use_ddp = globals().get("use_ddp", False)
num_gpus = globals().get("num_gpus", 0)
train_root_ddp = globals().get("train_root")
val_root_ddp = globals().get("val_root")
data_pt_path = globals().get("data_pt_path")
val_pt_path = globals().get("val_pt_path")
val_fraction = globals().get("val_fraction", 0.1)

_ddp_with_folders = use_ddp and num_gpus and train_root_ddp is not None and val_root_ddp is not None
_ddp_with_pt = use_ddp and num_gpus and data_pt_path is not None

if (_ddp_with_folders or _ddp_with_pt) and torch.cuda.device_count() >= num_gpus:
    try:
        torch.multiprocessing.set_start_method("spawn", force=True)
    except RuntimeError:
        pass
    num_epochs_ddp = getattr(config, "num_epochs", 10)
    checkpoint_dir_ddp = Path(getattr(config, "checkpoint_dir", "checkpoints"))
    if _ddp_with_pt:
        torch.multiprocessing.spawn(
            run_ddp_worker_pt,
            args=(num_gpus, config.to_dict(), Path(data_pt_path), val_pt_path and Path(val_pt_path), val_fraction, num_epochs_ddp, checkpoint_dir_ddp),
            nprocs=num_gpus,
            join=True,
        )
    else:
        torch.multiprocessing.spawn(
            run_ddp_worker_folders,
            args=(num_gpus, config.to_dict(), Path(train_root_ddp), Path(val_root_ddp), num_epochs_ddp, checkpoint_dir_ddp),
            nprocs=num_gpus,
            join=True,
        )
    print("done (DDP).")
else:
    # --- Single-device setup (use your existing config and model) ---
